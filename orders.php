...
// TEMPORARY TEST ROUTE -- visit /orders.php?test=1 to verify all 3
// fixes (dedup, item validation, cancelStatus) end-to-end. Remove once
// confirmed working.
if (isset($_GET['test'])) {
    header('Content-Type: text/plain; charset=utf-8');
    $number = 'TEST-' . time();

    function makeSampleOrder($number, $code, $price = 749) {
        return [
            'number' => $number,
            'cartNumber' => $number . '-cart',
            'sum' => $price,
            ...
            'delivery' => ['warehouseNumber' => '1', 'settlementName' => 'Львів'],
            'paymentType' => 'mono--card',
            'payment' => [],
            'items' => [['code' => $code, 'quantity' => 1, 'price' => $price, 'originalPrice' => $price, 'discountAmount' => 0, 'discountPercent' => 0, 'amount' => $price]],
            'callbackUrl' => null,
            'isTest' => true,
        ];
    }

    function simulateCreate($mono) {
        if (empty($mono['number'])) return [400, ['error' => 'number required']];
        $existingId = findExistingKeycrmOrderByNumber($mono['number']);
        if ($existingId !== null) return [200, ['id' => (string)$existingId]];
        $missingCode = findMissingItemCode($mono['items'] ?? []);
        if ($missingCode !== null) return [409, ['message' => "Product with code {$missingCode} does not exist", 'code' => 'ITEM_NOT_FOUND']];
        $keycrmBody = buildKeycrmCreateRequest($mono);
        [$httpCode, $result] = keycrmRequest('POST', '/order', $keycrmBody);
        if ($httpCode >= 200 && $httpCode < 300 && isset($result['id'])) return [201, ['id' => (string)$result['id']]];
        return [500, ['error' => $result]];
    }

    echo "=== ТЕСТ 1: Дубль замовлення (той самий number двічі) ===\n";
    $order1 = makeSampleOrder($number, '41130');
    [$c1, $r1] = simulateCreate($order1);
    echo "1-й запит: HTTP $c1 -> " . json_encode($r1) . "\n";
    [$c2, $r2] = simulateCreate($order1);
    echo "2-й запит (той самий number): HTTP $c2 -> " . json_encode($r2) . "\n";
    echo ($r1['id'] === $r2['id'] && $c2 === 200) ? "✅ РЕЗУЛЬТАТ: дублікат НЕ створено, id збігається\n\n" : "❌ РЕЗУЛЬТАТ: дублікат все ще створюється!\n\n";

    echo "=== ТЕСТ 2: Товар, якого не існує ===\n";
    $order2 = makeSampleOrder('TEST-FAKE-' . time(), '110000000022');
    [$c3, $r3] = simulateCreate($order2);
    echo "HTTP $c3 -> " . json_encode($r3) . "\n";
    echo ($c3 === 409 && ($r3['code'] ?? '') === 'ITEM_NOT_FOUND') ? "✅ РЕЗУЛЬТАТ: правильно відхилено (ITEM_NOT_FOUND)\n\n" : "❌ РЕЗУЛЬТАТ: замовлення створилось, хоча не мало б!\n\n";

    echo "=== ТЕСТ 3: cancelStatus після скасування ===\n";
    $order3 = makeSampleOrder('TEST-CANCEL-' . time(), '41130');
    [$c4, $r4] = simulateCreate($order3);
    if ($c4 === 201 && isset($r4['id'])) {
        $orderId = $r4['id'];
        echo "Створено замовлення id=$orderId, скасовуємо...\n";
        [, $existing] = keycrmRequest('GET', "/order/{$orderId}");
        keycrmRequest('PUT', "/order/{$orderId}", ['status_id' => KEYCRM_CANCEL_STATUS_ID, 'manager_comment' => trim(($existing['manager_comment'] ?? '') . "\nТЕСТОВЕ скасування")]);
        [, $updated] = keycrmRequest('GET', "/order/{$orderId}");
        $response = buildMonoOrderResponse($updated ?: $existing);
        $response['cancelStatus'] = 'canceled';
        echo json_encode($response, JSON_UNESCAPED_UNICODE) . "\n";
        echo ($response['cancelStatus'] === 'canceled') ? "✅ РЕЗУЛЬТАТ: cancelStatus заповнено правильно\n" : "❌ РЕЗУЛЬТАТ: cancelStatus все ще порожній!\n";
    } else {
        echo "Не вдалось створити тестове замовлення для цього тесту\n";
    }
    exit;
}
